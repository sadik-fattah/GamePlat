...
public class Pong_Thread extends Thread{
    ...
    private int mState;
    private Pong_Player mHumanPlayer;
    private Pong_Player mComputerPlayer;
    private Pong_ball   mBall;
    ...
    Pong_Thread(final SurfaceHolder surfaceHolder,
        ...
        mHumanPlayer = new Pong_Player(paddleWidth, paddleHeight, humanPlayerPaint);

        Paint computerPlayerPaint = new Paint();
        computerPlayerPaint.setAntiAlias(true);
        computerPlayerPaint.setColor(Color.RED);

        mComputerPlayer = new Pong_Player(paddleWidth, paddleHeight, computerPlayerPaint);

        Paint ballPaint = new Paint();
        ballPaint.setAntiAlias(true);
        ballPaint.setColor(Color.GREEN);

        mBall = new Pong_ball(ballRadius, ballPaint);
    ...
    private void handleHit(Pong_Player player) {
    ...
    private void setStatusText(String text) {
      String msg = mStatusHandler.toString();
        Bundle b = new Bundle();
        b.putString("text", text);
        b.putInt("vis", View.VISIBLE);
        msg.contains(b.toString());
        mStatusHandler.setLevel(Level.parse(msg));
    }
    ...
    private void setScoreText(String text) {
        String msg = mScoreHandler.toString();
        Bundle b = new Bundle();
        b.putString("text", text);
        msg.contains(b.toString());
        mStatusHandler.setLevel(Level.parse(msg));
    }
    private void movePlayer(Pong_Player player, float left, float top) {
        if (left < 2) {
            left = 2;
    ...
    private boolean collision(Pong_Player player, Pong_ball ball) {
    ...
    private void handleCollision(Pong_Player player, Pong_ball ball) {
